...
- **Maps 83 societies** across ~24 specialties and five regions — the US, Europe (pan-European bodies), the UK, Poland and international organisations (ACG, AGA, AASLD, BSG, ECCO, UEG, EASL, ESC, KDIGO, GOLD, USPSTF, PTG-E, PTN, PTDerm, EADV…). Each society carries the guideline-index URL its documents are published on.
- **Indexes 110 current guidelines** (127 including prior editions) with structured treatment summaries, spanning 12 specialties (gastroenterology, hepatology, cardiology, nephrology, dermatology, pulmonology, oncology, urology, ophthalmology, diabetology, infectious disease and primary-care/preventive medicine) across all five regions.
- **PubMed-verified provenance** — where a guideline is journal-published and PubMed-indexed, its record carries a verified DOI (over half the current catalog); the PubMed MCP server is used to locate guidelines from societies whose own sites are unreachable, confirm citations, and check copyright before paraphrasing. Everything remains a hand-curated demo with a persistent "verify against source" framing.
...
- **Region comparison** — a US ↔ Europe ↔ UK ↔ Poland matrix aligned by therapy line, highlighting where societies **agree** vs. **diverge** (drug names matched across languages). Ulcerative colitis and Crohn's now compare across all four regions (ACG/AGA · ECCO · BSG · PTG-E); hepatitis B and hypertension compare US ↔ EU (↔ PL).
