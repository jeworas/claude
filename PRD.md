...
### F5b — Drug cross-reference (drug → conditions)
The inverse of the condition view. Searching a medicine (e.g. *infliximab*) resolves to a drug page listing **every condition it is recommended for**, and within each condition every guideline that names it — with therapy line, evidence grade and standard dosing. Drug names are canonicalised with the same language-normalised key as the comparison view, so a Polish *infliksymab* and an English *infliximab* (and parenthetical dosing spellings) collapse to one medicine. Browsable at `/drugs`; a search for a drug shows a "Medicine" banner linking straight to the cross-reference. It surfaces **no new clinical facts** — it re-groups the curated recommendation/dosing data by drug, so it stays a reference index rather than advice.
